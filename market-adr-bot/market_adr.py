"""
코인 시장 ADR 봇 (OKX 현물 기준)
─────────────────────────────────────────────
시총 상위 200개 코인을 세 그룹으로 나눠 지난 6시간 등락 기준 ADR을 계산하고,
...
데이터 흐름:
  ...
  2. CoinGecko markets → 시총 상위 200개 코인 순위 + 심볼
  3. OKX instruments → OKX에 상장된 USDT 현물 목록
  4. (각 코인) OKX 6시간 전/현재 종가 → 6시간 등락 계산
     (ADR은 시장 분위기 측정이므로 현물 가격 사용)

* 바이낸스는 GitHub Actions에서 451 지역 차단으로 사용 불가 → OKX 단독.
6시간 간격 실행 (cron-job.org). 측정 기간도 6시간(=15분봉 24봉 전과 비교).
"""
...
def compute_group_adr(coins, okx_symbols, low_rank, high_rank):
    ...
    return {
        ...
        "not_on_okx": not_on_okx, "fetch_fail": fetch_fail,
    }
...
def main():
    ...
    # 진단 (로그 전용)
    print("\n  --- 진단: 집계 개수 ---")
    print(f"  [대형] {large['checked']}개  [중형] {mid['checked']}개  [소형] {small['checked']}개")
    none_all = large['not_on_okx'] + mid['not_on_okx'] + small['not_on_okx']
    print(f"  OKX에 없어 제외 ({len(none_all)}개): {', '.join(none_all) or '없음'}")
    print("  ----------------------\n")
